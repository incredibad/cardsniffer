...
- **Auth**: user accounts with admin/regular roles (`backend/auth.py`, `backend/routers/auth.py`, `backend/routers/users.py`). Session-based via an HttpOnly cookie backed by a server-side `auth_sessions` table (not a JWT — revocable on logout/deletion). The whole app is gated behind login (`frontend/src/App.jsx`) — an unauthenticated visitor sees only the full-page `Login` component and gets nothing else, including `/search` and `/stores`, which require a session server-side too (`require_user` in `backend/auth.py`). First load with no accounts shows that same screen as an admin-creation form instead (`setup_required` from `/auth/status`). Stores, VPN Proxy, Logs, and Users management remain admin-only on top of that.
- **VPN proxy**: scraper requests optionally route through an HTTP proxy — `vpn_proxy_url` (Settings → Admin → Network), seedable via the `VPN_PROXY_URL` env var. `docker-compose.yml` has a commented-out gluetun sidecar for setups with no existing proxy.
